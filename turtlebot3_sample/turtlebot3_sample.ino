...
#ifdef DEBUG
  if ((t - t tttTime[5]) >= (1000 / DEBUG_LOG_FREQUENCY))
  {
    sendDebuglog();
    tTime[5] = t;
  }
...
void publishIRDataMsg(void)
{
  irm_msg.data = mfwdIrVal.distance();
  irl_msg.data = lfwdIrVal.distance();
  irr_msg.data = rfwdIrVal.distance();

  //Serial.println(analogRead(A1));

  irm_pub.publish(&irm_msg);
  irl_pub.publish(&irl_msg);
  irr_pub.publish(&irr_msg);
}
...
void sendDebuglog(void)
{
  ...
  DEBUG_SERIAL.println("---------------------------------------");
  DEBUG_SERIAL.print("Bumper : ");
  DEBUG_SERIAL.println(sensors.checkPushBumper());
  DEBUG_SERIAL.print("Cliff : ");
  DEBUG_SERIAL.println(sensors.getIRsensorData());
  DEBUG_SERIAL.print("Sonar : ");
  DEBUG_SERIAL.println(sensors.getSonarData());
  DEBUG_SERIAL.print("Illumination : ");
  DEBUG_SERIAL.println(sensors.getIlluminationData());

  DEBUG_SERIAL.println("---------------------------------------");
  DEBUG_SERIAL.println("OpenCR SENSORS");
  DEBUG_SERIAL.println("---------------------------------------");
  DEBUG_SERIAL.print("Battery : ");
  DEBUG_SERIAL.println(sensors.checkVoltage());
